#This is the implementation of  EM on spatiotemporal theme pattern mining
#we ll consider each twitter as a document in the test
from __future__ import division
from readDir1 import DocidWordFreqTimeLocationDictFromDir, dir1
...
timeLocationStamps = dictionaryFile[2]
#print timeLocationStamps
#print wordDict.keys()
#count total number of words in the collection
timeLocationStampToDocIdDict = dictionaryFile[3]
timeLocationStampsNum = len(timeLocationStampToDocIdDict) 
timeToLocationDict = dictionaryFile[4]
locationToTimeDict = dictionaryFile[5]
...
#
prob_theta_timeloc = {}
for tl in timeLocationStampToDocIdDict.keys():
...
# calculate em
#===============================================================================
log_prob_C_new = log_prob_C_old=-10000000
        ...
        for word in dict2[docId]['words'].keys():
            ...
            for j in range(1, kmax):
                try:
                    ...
                    tempTl = timeLocationStamps[docId]#the time and location information corresponding to docID
            ...
            for j in range(1, kmax):
                tempTl = timeLocationStamps[docId]
                denominatorY = (1 - lambdaTl) * prob_theta_doc[docId][j] \
                + lambdaTl * prob_theta_timeloc[tempTl][j]
#                if denominatorY == 0:
    ...
    for docId in dict2.keys():
        ...
        for j in range(1, kmax): 
            ...
            for word in dict2[docId]['words'].keys():
                ...
                * (1 - prob_theta_timeloc[tempTl][j])#wrong!!
...
#            xigmaThetaD = 0.000001#initialization
    ...
    #(2)
    for stamp in timeLocationStampToDocIdDict.keys(): 
        for docId in timeLocationStampToDocIdDict[stamp]:
            xigmaThetaJTL = {} 
            for j in range(1, kmax):
                ...
                for word in dict2[docId]['words'].keys():
                    ...
                    * prob_theta_timeloc[stamp][j]#wrong!!
